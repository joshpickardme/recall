...
export default function Header({setIsThin}) {
    ...
    if(status != "authenticated") {
        ...
        return(
            <>
                <div onClick={updateThin} className={`${thin ? "" : "bg-black bg-opacity-60 fixed min-h-full min-w-full"} transition-all`}></div>
                <header className={`fixed bg-[var(--purple)] ${thin ? "w-20" : "w-full sm:w-64"} shadow-xl h-screen transition-all z-50`}>
                    <main className="flex flex-col items-center h-full z-10 shadow-2xl">
                        ...
                        <div className="flex flex-col justify-between w-full h-full mt-12 mb-12">
                            <div className="flex flex-col justify-center min-w-full">
                                ...
                                <NavButton link={"/decks/manage"} active={false} icon={<IoOptionsSharp />} text={thin ? " " : "Manage Decks"}></NavButton>
                            </div>
                            <div className="flex flex-col justify-center min-w-full">
